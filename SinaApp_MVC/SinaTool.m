...
//
#define APP_KEY @"2704017840"
#define APP_SECRET @"3a11cb8287ad682444869429b35f09de"
...
//    https://api.weibo.com/2/statuses/home_timeline.json
    ...
    [manager GET:@"https://api.weibo.com/2/statuses/home_timeline.json" parameters:@{TOKEN_KEY:tokenString} progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        ...
        //将字典转换成model
        ZYweibo *model = [ZYweibo modelObjectWithDictionary:responseObject];
...
-(void)sendHomeTimeLineWithToken:(NSString*)tokenString withText:(NSString*)text withImage:(NSData*)imageData
{
    //    https://api.weibo.com/2/statuses/home_timeline.json
    
    AFHTTPSessionManager *manager = [AFHTTPSessionManager manager];
    
    [manager POST:@"https://upload.api.weibo.com/2/statuses/upload.json" parameters:@{TOKEN_KEY:tokenString,@"status":text} constructingBodyWithBlock:^(id<AFMultipartFormData>  _Nonnull formData) {
        [formData appendPartWithFileData:imageData name:@"pic" fileName:@".png" mimeType:@"/png"];
